...
//                animation.start();
    ...
    private void attemptLogin() {
        ...
        String card = mEmailView.getText().toString();
...
//            cancel = true;
//        } else if (!isCardNumberValid(card)) {
...
//        }
        ...
        if (cancel) {
            ...
            showProgress(true);
            mAuthTask = new UserLoginTask(card, password);
            mAuthTask.execute((Void) null);
            //switch activity
        }
    }

    private boolean isCardNumberValid(String card) {
        //TODO: Replace this with your own logic
        if(card.matches("[0-9]+") && card.length() == 12)
            return true;
        return false;
    }
